...
function getRandCard(){
  var tmp = ["2", "3", "4", "5", "6", "7", "8", "9", "10", "jack", "queen", "king", "ace"];
  shuffle(tmp);
  var y= Math.floor(Math.random() * 4);
  if(uniqs.includes(tmp[0]+"_of_"+cardsSuits[y])){
    getRandCard()
  }
  else {
    uniqs.push(tmp[0]+"_of_"+cardsSuits[y]);
    return (tmp[0]+"_of_"+cardsSuits[y]);
  }
}
...
{
  if(document.getElementsByClassName('picked-cards-area')[0].childNodes.length > 0)
  {
    resetButton();
  }
  var z=getRandCard();
  ...
  var table_cards = [];
  for(var i =0;i<5;i++){
    table_cards.push(getRandCard())
  }
  table_cards.forEach(function(card){
    pickCard(document.getElementsByClassName(card)[0]);
  })
  var r=document.getElementsByClassName(z)[0];
  var r1=document.getElementsByClassName(z1)[0];
  pickCard2(r);
  pickCard2(r1);
}
...
$(document).on("mouseover", ".hand-list-element", function() {
